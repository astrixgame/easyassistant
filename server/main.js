...
var con = mysql.createConnection({ host: "localhost", user: "root", password: "", database: "assistant" });
var clients = new Set();
...
con.connect(function(err) {
    ...
    sql_query("SELECT v FROM configuration WHERE k='loxone_address' OR k='loxone_username' OR k='loxone_password';").then(function(result) {
        loxoneConnection(result[0]["v"], result[1]["v"], result[2]["v"]);
    });
                ...
                socket.send("jdev/sps/enablebinstatusupdate").then(function(respons) {
                    ...
                    */
                    ...
                    wss.on('connection', function(ws) {
                        ...
                        sql_query("SELECT v FROM configuration WHERE k='radio_station' OR k='radio_stations';").then(function(result) {
                            var c_radio = result[1]["v"].split(";")[parseInt(result[0]["v"])-1].split(",");
                            ws.send(JSON.stringify({ module: "radio", action: "change", image: c_radio[1], title: c_radio[0] }));
                        });
                        ...
                        sql_query("SELECT v FROM configuration WHERE k='openweather_weather';").then(function(result) {
                            var weatherObject = JSON.parse(result[0]["v"]);
                            ...
                            switch(dt["module"]) {
                                case "radio":
                                    if(dt["action"] == "play") {
                                        ...
                                        sql_query("SELECT v FROM configuration WHERE k='radio_station' OR k='radio_playing' OR k='radio_stations';").then(function(result) {
                                            var current_radio = parseInt(result[0]["v"]);
                                            if(current_radio > 1) {
                                                setConfig("radio_station",""+(current_radio-1));
                                                var c_radio = result[2]["v"].split(";")[current_radio-2].split(",");
                                                stop();
                                                sendMessage(JSON.stringify({ module: "radio", action: "change", image: c_radio[1], title: c_radio[0] }));
                                                playAudioStream(c_radio[2]);
                                                if(result[1]["v"] == "yes") play();
                                            }
                                        });
                                    } else if(dt["action"] == "next") {
                                        sql_query("SELECT v FROM configuration WHERE k='radio_station' OR k='radio_playing' OR k='radio_stations';").then(function(result) {
                                            var current_radio = parseInt(result[0]["v"]);
                                            if(current_radio < (result[2]["v"].match(/;/g) || []).length+1) {
                                                setConfig("radio_station",""+(current_radio+1));
                                                var c_radio = result[2]["v"].split(";")[current_radio].split(",");
                                                stop();
                                                sendMessage(JSON.stringify({ module: "radio", action: "change", image: c_radio[1], title: c_radio[0] }));
                                                playAudioStream(c_radio[2]);
                                                if(result[1]["v"] == "yes") play();
...
async function sendVlcRequest(url) {
    try { await axios.get(url, { auth: { username: '', password: '1234' }}); } catch(e) {}
}
